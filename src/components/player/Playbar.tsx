...
export function Playbar({
  ...
  const checkIfLiked = useLibraryStore((state) => state.checkIfLiked);

  // Check if track is liked whenever track changes
  useEffect(() => {
    if (track && track.id) {
      checkIfLiked(track.id).then(setIsLiked);
    }
  }, [track?.id, checkIfLiked]);

  const handleLikeToggle = async () => {
    if (!track) return;
    const newState = await toggleLike(track, isLiked);
    setIsLiked(newState);
  };

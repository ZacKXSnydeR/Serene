...
export const LikedSongsPage: React.FC = () => {
  ...
  const fetchLikedSongs = async (currentLimit: number) => {
    ...
    setIsLocal(false);
    try {
      const res = await fetch(`${getBaseUrl()}/library/liked?limit=${currentLimit}`);
      ...
      setIsLocal(true);
      try {
        const localTracks: any[] = await invoke('get_local_liked_songs');
    ...
    <div className="w-full flex flex-col items-center pb-24">
      ...
      <div className="w-full px-8 mt-12">
        <TrackGrid 
          ...
          title={isLocal ? "Local Liked Songs" : "Liked Songs"}
          onArtistClick={(id) => navigate(`/artist/${id}`)}
        />
        
        {!isLocal && tracks.length >= limit && (
          <div className="flex justify-center mt-8 mb-12">
            <button 
              onClick={() => setLimit(l => l + 30)}
              disabled={isLoading}
              className="px-6 py-2 rounded-full border border-white/10 text-white/70 hover:text-white hover:bg-white/5 transition-colors"
            >
